...
import aurora_dnn.input_data as aurora_input
...
tf.app.flags.DEFINE_string('data_dir', '/home/aurora/hdd/workspace/PycharmProjects/data/aurora2/bin_files_2_3_class_256*256/',
                           """Path to the aurora data directory.""")

# Global constants describing the CIFAR-10 data set.
IMAGE_SIZE = aurora_input.IMAGE_SIZE
NUM_CLASSES = aurora_input.NUM_CLASSES
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = aurora_input.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = aurora_input.NUM_EXAMPLES_PER_EPOCH_FOR_EVAL
...
def distorted_inputs():
  ...
  return aurora_input.distorted_inputs(data_dir=FLAGS.data_dir,
...
def inputs(eval_data):
  ...
  if not FLAGS.data_dir:
    raise ValueError('Please supply a data_dir')
  return aurora_input.inputs(eval_data=eval_data, data_dir=FLAGS.data_dir,
                              batch_size=FLAGS.batch_size)
...
def inference(images):
  ...
  with tf.variable_scope('conv1') as scope:
    kernel = _variable_with_weight_decay('weights', shape=[3, 3, 1, 64], stddev=1e-4, wd=0.0)
  ...
  # droupout
  norm1 = tf.nn.dropout(norm1, DROPOUT_VALUE)
  ...
  norm2 = tf.nn.dropout(norm2, DROPOUT_VALUE)
  ...
  norm3 = tf.nn.dropout(norm3, DROPOUT_VALUE)


  # local4
  with tf.variable_scope('local4') as scope:
    # Move everything into depth so we can perform a single matrix multiply.
    # reshape = tf.reshape(norm3, [-1, 4*4*256])
    dim = 1
    for d in norm3.get_shape()[1:].as_list():
      dim *= d
    reshape = tf.reshape(norm3, [FLAGS.batch_size, dim])
  ...
  with tf.variable_scope('softmax_linear') as scope:
    weights = _variable_with_weight_decay('weights', [1024, NUM_CLASSES], stddev=1/192.0, wd=0.0)
